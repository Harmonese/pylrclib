...
def add_parser(subparsers) -> ArgumentParser:
    parser = subparsers.add_parser(
        "cleanse",
        help=get_text("cmd.cleanse.help"),
        description=get_text("cmd.cleanse.description"),
        epilog=get_text("cmd.cleanse.epilog"),
        formatter_class=RichHelpFormatter,
    )
    parser.add_argument(
        "paths", nargs="*", default=[],
        help=with_default(
            get_text("cmd.cleanse.arg.paths"),
            get_text("default.cwd_paths"),
        ),
    )
    parser.add_argument(
        "--lrc-dir", default=UNSET,
        help=with_default(
            get_text("cmd.cleanse.arg.lrc_dir"),
            get_text("default.cwd_lrc_dir"),
        ),
    )
    parser.add_argument(
        "--write", action="store_true",
        help=with_default(
            get_text("cmd.cleanse.arg.write"),
            get_text("default.disabled"),
        ),
    )
    parser.add_argument(
        "--preview-lines", default=UNSET,
        help=with_default(
            get_text("cmd.cleanse.arg.preview_lines"),
...
def run(args: Namespace, lang: str) -> int:
    ...
    if not files:
        log_warn(get_text("cmd.cleanse.no_lrc_found"))
        return 0
    ...
    log_info(get_text("cmd.cleanse.summary", summary=", ".join(f"{key}={value}" for key, value in sorted(stats.items()))))
    return 0
